...
| **Auto-Apply** | `apply/` | Claude Code navigates forms, fills fields, uploads docs, answers questions, submits |
...
- **`apply/prompt.py`**: Generates Claude Code prompts for auto-apply. Used by MCP server at runtime
...
- `LLM_MODEL`: e.g., "gemini-3.1-flash-lite", "gpt-4o", "claude-3-5-sonnet-20241022"
...
The `apply/` stage uses Claude Code CLI with an MCP (Model Context Protocol) server for browser automation:
...
- Claude Code (via MCP) navigates forms, fills fields, uploads documents, and submits
